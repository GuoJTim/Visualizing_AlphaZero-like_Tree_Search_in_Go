...
const colorScale = d3.scaleLinear()
    .domain([0.008, 0.009, 0.01, 0.015, 0.05])
    .range(['red', 'purple', 'blue', 'green', 'yellow']);
...
function drawBlueGoBoard(container, boardData, heatmapData, policycolorData, size = 300) {
    ...
    for (let i = 0; i < boardSize; i++) {
        for (let j = 0; j < boardSize; j++) {
            if (heatmapData[i][j] === 1) {
                const colorpolicy = policycolorData[i][j];
                const fillcolor = colorScale(colorpolicy);
